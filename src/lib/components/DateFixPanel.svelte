<script lang="ts">
  import type { EntryView, OffsetView, Provider } from "$lib/api";
  import { describeShift, fromInputValue, toInputValue } from "$lib/dates";
  import { FALLBACK_PROVIDER, offeredBy, pickProvider } from "$lib/sources";
  ...
  let target = $state("");
  let anchored = $state("");
  let provider = $state<Provider>(FALLBACK_PROVIDER);

  const anchor = $derived(entries[0] ?? null);
  const sources = $derived(entries.map((entry) => entry.source));

  const providers = $derived(
    offeredBy(entries.map((entry) => entry.candidates.map((c) => c.provider))),
  );

  $effect(() => {
    const kept = pickProvider(providers, provider);
    if (kept !== null && kept !== provider) provider = kept;
  });
...
<div class="panel">
  ...
  {#if offsets.length > 0}
    <div class="runs">
      <p class="faint hint">
        These files read the same amount away from another source, which is the shape of a clock
        that was never set rather than a wrong date one at a time.
      </p>
      {#each offsets as run (run.reference + run.seconds)}
        <div class="run">
          <span class="told">{run.told}</span>
          <button class="ghost" disabled={busy} onclick={() => onShift(run.sources, -run.seconds)}>
            Correct {run.files}
          </button>
        </div>
      {/each}
