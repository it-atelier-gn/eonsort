<script lang="ts" generics="Id extends string">
  ...
  interface Props {
    ...
    sorted?: { id: Id; down: boolean } | null;
    onSort?: (id: Id) => void;
  }

  let { set, order, grid, onReorder, onResize, sorted = null, onSort }: Props = $props();
...
<div class="head" style="grid-template-columns: {grid}" role="row">
  {#each order as id (id)}
    <span
      ...
      draggable="true"
      title={onSort
        ? "Click to sort, drag to reorder, or use the arrow keys"
        : "Drag to reorder, or use the arrow keys"}
      aria-sort={sorted?.id === id ? (sorted.down ? "descending" : "ascending") : "none"}
      onclick={() => onSort?.(id)}
    ...
    >
      {columnOf(set, id).label}{#if sorted?.id === id}<span class="arrow"
          >{sorted.down ? "▾" : "▴"}</span
        >{/if}
      <button
        type="button"
