...
describe("Undoing a turn", () => {
  ...
  it("turns a quarter the other way", () => {
    expect(undone("rotate90")).toBe("rotate270");
    expect(undone("rotate270")).toBe("rotate90");
    expect(undone("rotate180")).toBe("rotate180");
    expect(undone("none")).toBe("none");
  });

  it("leaves a mirroring alone, because doing it twice is doing nothing", () => {
    expect(undone("flip_h")).toBe("flip_h");
    expect(undone("flip_v")).toBe("flip_v");
    expect(undone("transpose")).toBe("transpose");
    expect(undone("transverse")).toBe("transverse");
  });

  it("puts every turn back where it started", () => {
    for (const transform of every) {
      expect(undone(undone(transform))).toBe(transform);
    }
  });
});
