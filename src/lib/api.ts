...
export type DateChoice =
  { kind: "candidate"; provider: Provider } | { kind: "manual"; taken: string };
...
export const startCopy = (preserveTimes: boolean, stampDate: boolean, writeSidecars: boolean) =>
  invoke<void>("start_copy", { preserveTimes, stampDate, writeSidecars });
export const findDuplicates = () => invoke<DuplicateReport>("find_duplicates");
export const removeExtraCopies = () =>
  invoke<PruneReport>("remove_extra_copies");

export const findBursts = () => invoke<BurstView[]>("find_bursts");
export const findLookalikes = () => invoke<LookalikeView[]>("find_lookalikes");

export const startVerify = (compareHashes: boolean) =>
  invoke<void>("start_verify", { compareHashes });
export const openPlan = (path: string) =>
  invoke<PlanSummary>("open_plan", { path });
...
export const picturesLike = (source: string) =>
  invoke<TagHit[]>("pictures_like", { source });

export const planOffsets = () => invoke<OffsetView[]>("plan_offsets");
...
export const previewFile = (path: string) =>
  invoke<Preview>("preview_file", { path });
export const saveScreenshot = () => invoke<string>("save_screenshot");
...
export interface GazetteerStatus {
  present: boolean;
  bytes: number;
  total: number;
  credit: string;
...
export const gazetteerStatus = () => invoke<GazetteerStatus>("gazetteer_status");
export const installGazetteer = () => invoke<void>("install_gazetteer");
export const cancelGazetteerInstall = () => invoke<void>("cancel_gazetteer_install");
export const removeGazetteer = () => invoke<void>("remove_gazetteer");
...
export const startTagging = (afresh = false) =>
  invoke<number>("start_tagging", { afresh });
export const cancelTagging = () => invoke<void>("cancel_tagging");
export const listTags = () => invoke<Record<string, Sighting>>("list_tags");
export const qualityModelStatus = () =>
  invoke<TagModelStatus>("quality_model_status");
export const installQualityModel = () => invoke<void>("install_quality_model");
export const cancelQualityInstall = () =>
  invoke<void>("cancel_quality_install");
export const searchPictures = (words: string) =>
  invoke<TagHit[]>("search_pictures", { words });
export const faceStatus = () => invoke<FaceStatus>("face_status");
export const listFaces = () => invoke<Record<string, Spot[]>>("list_faces");
export const startFaceHunt = (afresh = false) =>
  invoke<number>("start_face_hunt", { afresh });
export const cancelFaceHunt = () => invoke<void>("cancel_face_hunt");
export const listNames = () => invoke<NameCount[]>("list_names");
export const nameFace = (
  source: string,
  ord: number,
  label: string | null,
  spread: boolean,
) => invoke<NamedFaces>("name_face", { source, ord, label, spread });
