...
describe("Remembering how wide the folder list is", () => {
  it("keeps the divider inside the range a window can show", () => {
    expect(clampPane(10)).toBe(MIN_PANE);
    expect(clampPane(5000)).toBe(MAX_PANE);
    expect(clampPane(312.4)).toBe(312);
  });

  it("falls back to the usual width when nothing sensible was kept", () => {
    expect(cleanPane(null)).toBe(TREE_PANE);
    expect(cleanPane("wide")).toBe(TREE_PANE);
    expect(cleanPane(Number.NaN)).toBe(TREE_PANE);
  });

  it("takes a width that was kept", () => {
    expect(cleanPane(420)).toBe(420);
  });
});

describe("the preview pane width", () => {
  it("is held between a readable minimum and a sensible maximum", () => {
    expect(clampPreview(10)).toBe(MIN_PREVIEW);
    expect(clampPreview(5000)).toBe(MAX_PREVIEW);
    expect(clampPreview(412.6)).toBe(413);
  });

  it("falls back to its default when nothing sensible was remembered", () => {
    expect(cleanPreview(null)).toBe(PREVIEW_PANE);
    expect(cleanPreview("wide")).toBe(PREVIEW_PANE);
    expect(cleanPreview(Number.NaN)).toBe(PREVIEW_PANE);
    expect(cleanPreview(Number.POSITIVE_INFINITY)).toBe(PREVIEW_PANE);
  });

  it("keeps a width that was remembered", () => {
    expect(cleanPreview(500)).toBe(500);
  });

  it("starts inside its own bounds", () => {
    expect(PREVIEW_PANE).toBeGreaterThanOrEqual(MIN_PREVIEW);
    expect(PREVIEW_PANE).toBeLessThanOrEqual(MAX_PREVIEW);
  });
});

describe("the height of the picture above the details", () => {
  it("is held between a minimum and a maximum", () => {
    expect(clampVisual(10)).toBe(MIN_VISUAL);
    expect(clampVisual(5000)).toBe(MAX_VISUAL);
    expect(clampVisual(300.4)).toBe(300);
  });

  it("falls back to its default when nothing sensible was remembered", () => {
    expect(cleanVisual(null)).toBe(VISUAL_PANE);
    expect(cleanVisual("tall")).toBe(VISUAL_PANE);
    expect(cleanVisual(Number.NaN)).toBe(VISUAL_PANE);
  });

  it("keeps a height that was remembered", () => {
    expect(cleanVisual(420)).toBe(420);
  });

  it("starts inside its own bounds", () => {
    expect(VISUAL_PANE).toBeGreaterThanOrEqual(MIN_VISUAL);
    expect(VISUAL_PANE).toBeLessThanOrEqual(MAX_VISUAL);
  });
});
