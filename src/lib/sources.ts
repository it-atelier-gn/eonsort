...
export const SOURCES: Source[] = [
  {
    id: "gps",
    label: "GPS",
    hint: "The satellite clock, which a wrong camera clock cannot reach",
    weight: 48,
  },
  {
    id: "xmp",
...
export const FALLBACK_PROVIDER: Provider = "filesystem";
...
export function offeredBy(candidates: Provider[][]): Provider[] {
  return [...new Set(candidates.flat())].sort();
}

export function pickProvider(offered: Provider[], current?: Provider): Provider | null {
  if (offered.length === 0) return null;
  if (current !== undefined && offered.includes(current)) return current;
  return offered.includes(FALLBACK_PROVIDER) ? FALLBACK_PROVIDER : offered[0];
}
...
export const LADDER: number[] = SOURCES.map((source) => source.weight)
  .slice()
  .sort((a, b) => b - a);

export function defaultOrder(): Provider[] {
  return SOURCES.map((source) => source.id);
}

export function weightsFor(order: Provider[]): Weights {
  const ranked = listing(order);
  const held: Weights = {};
  ranked.forEach((id, at) => {
    held[id] = LADDER[Math.min(at, LADDER.length - 1)];
  });
  return held;
}

export function atDefaultOrder(order: Provider[]): boolean {
  return listing(order).join() === defaultOrder().join();
}
