...
fn transfer(
    ...
    if options.write_sidecars {
        let _ = crate::xmp_write::write(&destination, &sidecar_for(entry));
    }
...
pub fn sidecar_for(entry: &PlanEntry) -> crate::xmp_write::Sidecar {
    crate::xmp_write::Sidecar {
        taken: Some(entry.taken),
        at: crate::geocode::read(&entry.source),
        place: entry.place.clone(),
        tags: entry.tags.clone(),
        people: entry.subject.iter().cloned().collect(),
...
mod tests {
    ...
    fn landed_files(root: &Path, extension: &str) -> Vec<PathBuf> {
    ...
    #[test]
    fn a_copy_leaves_a_sidecar_beside_it_when_asked() {
        let fixture = Fixture::new(&[("a.jpg", b"payload")]);
        let options = CopyOptions {
            write_sidecars: true,
            ..CopyOptions::default()
        };
        execute(&fixture.plan, &options, &AtomicBool::new(false), &|_| {}).unwrap();

        let out = fixture.dir.path().join("out");
        let sidecars = landed_files(&out, "xmp");
        assert_eq!(sidecars.len(), 1, "{sidecars:?}");
        assert!(sidecars[0].to_string_lossy().ends_with("a.jpg.xmp"));

        let body = fs::read_to_string(&sidecars[0]).unwrap();
        assert!(body.contains("xmp:CreateDate="), "{body}");
        assert!(body.starts_with("<?xpacket begin="), "{body}");
    }

    #[test]
    fn no_sidecar_is_written_unless_it_is_asked_for() {
        let fixture = Fixture::new(&[("a.jpg", b"payload")]);
        execute(
            &fixture.plan,
            &CopyOptions::default(),
        ...
        let out = fixture.dir.path().join("out");
        assert!(landed_files(&out, "xmp").is_empty());
    }

    #[test]
    fn the_sidecar_carries_what_the_entry_knows() {
        let entry = PlanEntry {
            ...
            tags: vec!["a beach".to_string()],
            subject: Some("Grandma".to_string()),
            ..PlanEntry::default()
        };

        let sidecar = sidecar_for(&entry);
        assert_eq!(sidecar.place.city.as_deref(), Some("Munich"));
        assert_eq!(sidecar.tags, vec!["a beach".to_string()]);
        assert_eq!(sidecar.people, vec!["Grandma".to_string()]);
