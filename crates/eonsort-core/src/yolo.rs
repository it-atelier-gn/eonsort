...
mod tests {
    ...
    #[test]
    fn each_size_asks_for_a_different_shape() {
        let all = [
            Multiples::n(),
            Multiples::s(),
            Multiples::m(),
    ...
    #[test]
    fn the_name_of_a_weight_file_picks_its_shape() {
        assert_eq!(Multiples::named("yolov8s"), Multiples::s());
        assert_eq!(Multiples::named("yolov8m"), Multiples::m());
        assert_eq!(Multiples::named("yolov8l"), Multiples::l());
        assert_eq!(Multiples::named("yolov8x"), Multiples::x());
    }

    #[test]
    fn an_unknown_name_falls_back_to_the_smallest() {
        assert_eq!(Multiples::named("yolov8n"), Multiples::n());
        assert_eq!(Multiples::named("nonsense"), Multiples::n());
    }

    #[test]
    fn every_variant_in_the_catalog_has_a_shape_of_its_own() {
        let shapes: Vec<Multiples> = crate::catalog::of(crate::catalog::Kind::Upright)
            .map(|v| Multiples::named(v.id))
            .collect();
        for (index, left) in shapes.iter().enumerate() {
            for right in &shapes[index + 1..] {
                assert_ne!(left, right, "two catalog variants share one shape");
            }
        }
    }
}
