...
fn reencoded(source: &Path, bytes: &[u8], transform: Transform) -> Result<Vec<u8>> {
    ...
    let turned = match transform {
        Transform::None => decoded,
        Transform::Rotate90 => decoded.rotate90(),
        Transform::Rotate180 => decoded.rotate180(),
        Transform::Rotate270 => decoded.rotate270(),
        Transform::FlipH => decoded.fliph(),
        Transform::FlipV => decoded.flipv(),
        Transform::Transpose => decoded.fliph().rotate270(),
        Transform::Transverse => decoded.fliph().rotate90(),
    };
