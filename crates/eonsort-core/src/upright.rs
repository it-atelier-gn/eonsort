...
pub fn total_bytes_of(variant: &crate::catalog::Variant) -> u64 {
    weights::total_bytes(variant.weights)
}

pub fn present_bytes_of(dir: &Path, variant: &crate::catalog::Variant) -> u64 {
    weights::present_bytes(dir, variant.weights)
}

pub fn installed_of(dir: &Path, variant: &crate::catalog::Variant) -> bool {
    weights::installed(dir, variant.weights)
}

pub fn remove_of(dir: &Path, variant: &crate::catalog::Variant) -> Result<()> {
    weights::remove(dir, variant.weights)
}
...
pub fn download_of(
    _dir: &Path,
    _variant: &crate::catalog::Variant,
    _cancel: &AtomicBool,
    _on_progress: &dyn Fn(UprightProgress),
) -> Result<()> {
    Err(Error::Upright(
...
pub fn download_of(
    ...
    weights::download(dir, variant.weights, cancel, on_progress)
}
...
impl Detector {
    ...
    pub fn load_of(_dir: &Path, _variant: &crate::catalog::Variant) -> Result<Detector> {
        Err(Error::Upright(
            "this build was made without the upright model".into(),
        ))
    }
...
impl Detector {
    pub fn load(dir: &Path) -> Result<Detector> {
        Self::load_of(
            dir,
            crate::catalog::default_of(crate::catalog::Kind::Upright),
        )
    }

    pub fn load_of(dir: &Path, variant: &crate::catalog::Variant) -> Result<Detector> {
        ...
        let weight = variant
            .weights
            .first()
            .ok_or_else(|| Error::Upright("that model has no weights".into()))?;
        let file = path_of(dir, weight);
        if !file.exists() {
        ...
        let model =
            YoloV8::load(builder, Multiples::named(variant.id), CLASSES.len()).map_err(oops)?;
        Ok(Detector { model })
    }
