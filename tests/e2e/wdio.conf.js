...
export const config = {
  ...
  onPrepare: () => {
    ...
    if (profile === "debug") build.splice(2, 0, "--debug");
    if (features) build.push("--features", features);
    spawnSync("npx", build, {
      cwd: repoRoot,
      stdio: "inherit",
      shell: true,
  ...
  before: async () => {
    await browser.waitUntil(
      async () => (await $$("[role=treeitem], .placeholder")).length > 0,
      {
        timeout: 180000,
...
function installedWebView2Version() {
  ...
  const output = execFileSync("reg", ["query", key, "/v", "pv"], {
    encoding: "utf8",
  });
  const match = output.match(/pv\s+REG_SZ\s+(\S+)/);
  if (!match) {
    throw new Error(
      "could not determine the installed WebView2 Runtime version",
    );
  }
  return match[1];
}
